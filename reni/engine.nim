...
proc restore(ctx: MatchContext, s: SavedState) =
  ctx.pos = s.pos
...
proc resetForRegex(
  ...
  # setLen grows if needed, keeps capacity on shrink — both avoid
  # reallocation when ``ctx`` is reused across calls.
