...
# ``test_engine_internals.nim`` and what the compiler's rewriting passes
# leave behind is in ``test_engine_passes.nim``.  Keep the three apart:
# ``refc`` allows 3500 module-level globals and a ``unittest`` body is
# module-level.
